#!/usr/bin/env bash
# plugin-cli 打包用的 cargo 垫片（仅 CI 的 Package 步骤经 PATH 注入）。
#
# plugin-cli 的 build_rust_backend 以 CARGO_TARGET_DIR=<dist/.build-rust-*>
...
if [ "$status" -eq 0 ] && [ -n "${CARGO_BUILD_TARGET:-}" ] && [ -n "${CARGO_TARGET_DIR:-}" ]; then
  binary="${SHIM_BINARY_NAME:-dbx-plugin-ssh}"
